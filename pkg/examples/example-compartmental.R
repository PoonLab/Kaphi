...
theta <- c(t.end=50, N=5000, beta=0.05, gamma=1/520, mu=1/3640, alpha=0)
set.seed(50)
...
x <- seq(100,2000, 100)    # (from, to, step)
res <- sapply(x, function(value) {
  theta <- c(t.end=50, N=value, beta=0.01, gamma=1/520, mu=1/3640, alpha=0)
...
# generate a plot
par(mar=c(5,5,2,2))
plot(x, res, type='b', xlab='N', ylab='Mean kernel distance', cex.lab=1.2, ylim=c(0,1))


# calculate kernel distances for varying beta
x <- seq(0.01,0.1, 0.01)    # (from, to, step)
res <- sapply(x, function(value) {
...
# generate a plot
par(mar=c(5,5,2,2))
plot(x, res, type='b', xlab='beta', ylab='Mean kernel distance', cex.lab=1.2, ylim=c(0,1))
