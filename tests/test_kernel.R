...
test.kernel.unnormalized <- function() {
    ...
    result <- tree.kernel(t1, t1, lambda=1, sigma=1, rho=1, normalize=0)
    expected <- 1 * 1 * ((1+1) * (1+1))
    checkEquals(expected, result)

    result <- tree.kernel(t1, t1, lambda=0.5, sigma=1, rho=1, normalize=0)
    expected <- 0.5 * 1 * ((1+0.5) * (1+0.5))
    checkEquals(expected, result)

    # RBF = 1 for all sigma because branch lengths are identical
    result <- tree.kernel(t1, t1, lambda=1.0, sigma=0.5, rho=1, normalize=0)
    expected <- 4
    checkEquals(expected, result)

    # t1 is nested within t2
    result <- tree.kernel(t1, t2, lambda=1.0, sigma=Inf, rho=1, normalize=0)
    expected <- 4
    checkEquals(expected, result)

    # t3 has the same topology as t1 but different branch lengths
    result <- tree.kernel(t1, t3, lambda=1.0, sigma=Inf, rho=1, normalize=0)
    expected <- 4  # no RBF penalty when sigma -> Infinity
    checkEquals(expected, result)

    # note sigma parameter for RBF *includes* the factor of two!
    result <- tree.kernel(t1, t3, lambda=1.0, sigma=1.0, rho=1, normalize=0)
...
test.kernel.normalized <- function() {
    result <- tree.kernel(t1, t1, lambda=1, sigma=0.5, rho=1, normalize=0)
    expected <- 4
    checkEquals(expected, result)
}
...
test.kernel.label <- function() {
    ...
    result <- tree.kernel(t1, t1, lambda=1, sigma=1, rho=1, label1=c(1, 2), label2=c(3, 4), gamma=0, normalize=0)
    expected <- 1
    checkEquals(expected, result)
    
    # half gamma
    cat("half gamma\n")
	result <- tree.kernel(t1, t1, lambda=1, sigma=1, rho=1, label1=c(1, 2), label2=c(3, 4), gamma=.5, normalize=0)
    expected <- 1.5 * 1.5
    checkEquals(expected, result)
    
    # same tree different labels, ignore mismatch
    cat("same tree different labels, ignore mismatch\n")
    result <- tree.kernel(t1, t1, lambda=1, sigma=1, rho=1, label1=c(1, 2), label2=c(3, 4), gamma=1, normalize=0)
    expected <- 4
    checkEquals(expected, result)
    
    # subset tree
    cat("subset tree\n")
    result <- tree.kernel(t1, t2, lambda=1, sigma=1, rho=1, label1=t1$tip.label, label2=t2$tip.label, gamma=0, normalize=0)
    expected <- 4
    checkEquals(expected, result)
    
    # subset tree, half gamma
    cat("subset tree, half gamma\n")
    result <- tree.kernel(t1, t2, lambda=1, sigma=1, rho=1, label1=t1$tip.label, label2=t2$tip.label, gamma=.5, normalize=0)
    expected <- 4
    checkEquals(expected, result)
    
    # subset tree, labels different
    cat("subset tree, labels different\n")
    result <- tree.kernel(t1, t2, lambda=1, sigma=1, rho=1, label1=c(1, 1), label2=c(2, 2, 2), gamma=0, normalize=0)
    expected <- 1
    checkEquals(expected, result)
    
    # subset tree, ignore mismatch
    cat("subset tree, ignore mismatch\n")
    result <- tree.kernel(t1, t2, lambda=1, sigma=1, rho=1, label1=c(1, 1), label2=c(2, 2, 2), gamma=0.5, normalize=0)
    expected <- 1.5 * 1.5
    checkEquals(expected, result)
    
    # subset tree, ignore mismatch
    cat("subset tree, ignore mismatch\n")
    result <- tree.kernel(t1, t2, lambda=1, sigma=1, rho=1, label1=c(1, 1), label2=c(2, 2, 2), gamma=1, normalize=0)
    expected <- 4
    checkEquals(expected, result)
    
    # subset tree, outgroup a
    cat("subset tree, outgroup a\n")
    result <- tree.kernel(t1, t2, lambda=1, sigma=1, rho=1, label1=c(1, 2), label2=c(1, 2, 1), gamma=0, normalize=0)
    expected <- 4
    checkEquals(expected, result)
}
